...
def main(args=None):
	...
	for idx, data in enumerate(dataloader_val):
		...
		with torch.no_grad():
			...
			scores, classification, transformed_anchors = retinanet(data['img'].cuda().float(),gen_anchor1)
		...
		res = cv2.resize(frame, dsize=(30, 30), interpolation=cv2.INTER_CUBIC)
		cv2.imwrite("myimage.jpg",frame)
		# new_img = F.upsample(frame, size_new=(30, 30), mode='bilinear')
		new_img = np.transpose(res, (2, 0, 1))
		new_img = np.repeat(new_img[np.newaxis, ...], ref_batch, axis=0)
		# new_img = torch.from_numpy(new_img)
		new_img = torch.from_numpy(new_img).float()
		# net = torch.load("model-epoch_traffic_light-2.pt")
		# new_img = new_img.unsqueeze(0)
		# input_batch = torch.repeat_interleave(frame, ref_batch, dim=0)
		# n = new_img.unsqueeze(0)
		input_batch=new_img
		# input_batch = transforms(input_batch)
		...
		img0 = img0.convert("RGB")
		# transforms.Compose([transforms.Resize((res,res),transforms.ToTensor()])
		...
		new_img =transform(img0)

		# new_img=transforms.Resize(30, interpolation=2)
		############################
		# third phase
